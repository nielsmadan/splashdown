...
mise and direnv only load a config once you trust/allow it, so splashdown runs that approval for you (`mise trust` / `direnv allow`): on `init` for a config it just created, and on every `sync` (including the post-checkout hook). This is why a fresh setup and every new worktree pick up `splashdown.env` with no manual step — a worktree's inherited config lives at a new path that its tool considers untrusted until approved. The one case left for you: if `init` edits a `.envrc`/`mise.toml` you already had, it won't auto-approve your own file (`splash init --no-sync` leaves it untouched for review); the next `splash sync` approves it.
