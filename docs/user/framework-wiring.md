...
Allocating a port doesn't always reach the running process. Most frameworks hardcode the port in one or two config files that override the env var, so splashdown carries per-framework wiring checks that detect those hardcoded points and (where safe) auto-patch them. `splash init` runs the wiring after scaffolding, checks the result again, and lists the files it changed, so a normal first setup does not need `splash doctor --fix` afterwards. `splash doctor` re-runs the checks anytime to verify, and `splash doctor --fix` re-applies the autofixes.
