...
- **Device/target rows** differ by scope. Default mode sources from the recipe + local catalog via `_gather_targets_declared`: it walks `merged_targets`, labels each variant by source, resolves the instance name, and reads live boot status. `all` mode sources device rows from the registry instead. Both paths use `device_health`, the same classifier that drives refresh, so model, image, name, and pinned-version drift cannot be reported clean when refresh would recreate the instance.
...
**`--check` (liveness revalidation).** Both the table and per-block paths accumulate counters into a shared `summary` dict: defunct checkouts, orphan devices whose underlying sim/AVD was deleted, stale devices whose recorded model/OS/image/name drifted from the declaration, undeclared registry rows, and missing declared devices. `_print_check_summary` routes each to the command that fixes it: `splash gc` for defunct rows, `splash target refresh` for orphan/stale/undeclared rows, and `splash run` for missing instances — or `Summary: all entries verified.` when clean.
