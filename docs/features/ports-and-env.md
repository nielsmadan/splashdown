...
After validation, provisioning computes the checkout's absolute path and git branch, then resolves every `[resources.*]` entry **in topological order** so a `template` that references another resource sees the referent's value (`src/splashdown/provisioning.py:55-60`, `topo_sort` at `src/splashdown/recipe.py:1062`). Resolution dispatches on `type`:

- **`port`** — reads a two-element `range = [lo, hi]` and calls `registry.allocate_port`, which returns the lowest free port in range, machine-wide (`src/splashdown/provisioning.py:63-70`, `src/splashdown/registry.py:182`). Allocation considers every other checkout's pinned ports from `ports.tsv` plus a live `bind()` probe on loopback (IPv4 and IPv6) so it also dodges ports held by non-splashdown processes (`src/splashdown/registry.py:200-207`, `_port_in_use` at `src/splashdown/registry.py:534`). An existing in-range pin for this checkout is kept as-is, even if currently bound — that bound port is almost always this checkout's own running dev server (`src/splashdown/registry.py:184-192`).
- **`uuid`** — returns the persisted value from the kv store if present, else mints a fresh `uuid4` and persists it; stable until `splash sync --force` (`src/splashdown/provisioning.py:71-76`).
- **`template`** — renders the `template = "..."` string against the current scope on every sync and refreshes its registry value, so referenced resources and expression edits propagate immediately (`src/splashdown/provisioning.py:83-89`).
- **`cwd` / `cwd-slug`** — the checkout directory name, raw or slugified (`src/splashdown/provisioning.py:77-82`).
- **`set`** — an externally supplied value: returns the persisted kv value, else the resource's `default`, else raises telling the user to run `splash env set NAME=VALUE` or add a `default` (`src/splashdown/provisioning.py:90`).
...
- **`splashdown-env`** — `splashdown.env`, written wholesale; splashdown owns the file (`write_splashdown_env`, `src/splashdown/provisioning.py:208`).
- **`envfile=PATH`** — any dotenv-format file; preserves non-managed lines, replacing only the keys splashdown manages (`write_envfile`, `src/splashdown/provisioning.py:214`).
- **`envrc`** — appends `export` lines to `.envrc.local` (`write_envrc`, `src/splashdown/provisioning.py:234`).
...
- `src/splashdown/registry.py:182` — `allocate_port`, lowest-free machine-wide + live `bind()` probe.
- `src/splashdown/registry.py:271` — `set_kv` / `get_kv` (`src/splashdown/registry.py:265`), persistence for non-port resources.
...
- **`set` with no value blocks the whole sync.** A `set` resource with neither a persisted value nor a `default` raises, aborting provision until the user runs `splash env set NAME=VALUE` or adds `default` (`src/splashdown/provisioning.py:37-43`, `:90-95`).
...
- **No-op syncs are silent.** `_write_if_changed` means a re-sync of an already-provisioned checkout collapses to "up to date" and touches no files — the expected output through lefthook/husky on `git pull --rebase` (`src/splashdown/provisioning.py:187`).
