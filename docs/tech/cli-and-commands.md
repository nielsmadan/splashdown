...
`_normalize_device_args` (`cli.py:284`) cleans up after the choice-less `dtype` slot. If `dtype` holds a non-type token and `variant` is empty, it shifts it over: `dtype, variant = None, dtype` (`cli.py:290`). Then it validates — anything still sitting in `dtype` that isn't a real `TARGET_TYPES` member raises `DeviceError` (`cli.py:291`). Type names win over equally-named variants: a variant literally named `simulator` is unreachable because the token is read as the type (see [Gotchas](#gotchas)). It is called from `main()` only for the four device verbs (`cli.py:345`).
