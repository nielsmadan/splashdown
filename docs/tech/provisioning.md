...
Entry: `provision()` at `provisioning.py:26`.

1. Locate `splashdown.toml` in `cwd`. **If it's missing, raise `FileNotFoundError`** (`provisioning.py:33-34`). Callers in `commands.py` catch this and turn it into a no-op exit 0, so the post-checkout hook is silent in non-splashdown repos (see Gotchas).
...
| `set` | reuse stored value, else fall back to `default`; **error if neither exists** | yes (kv) when defaulted |
   ...
   Unknown types raise `ValueError` (`provisioning.py:82`).

5. Each resolved value lands in `resolved[name]`; the dict is returned (`provisioning.py:83-84`).
...
Entry: `write_outputs()` at `provisioning.py:87`.

1. Group `resolved` by each resource's `writer` field, defaulting to `splashdown-env` (`provisioning.py:97`).
2. **Truncate guard**: if no resource targets `splashdown-env` anymore but the file still exists on disk, inject an empty group for it so the stale file gets emptied rather than left lying with values that contradict the recipe (`provisioning.py:99`).
3. For each `(writer, items)` group, dispatch (`provisioning.py:103`):
   ...
   - `envfile=<path>` → `write_envfile`, creating missing parent directories. The schema already requires a
     ...
     guard** (`provisioning.py:115`) also requires the resolved target to be
...
`_write_if_changed()` (`provisioning.py:175`) is the common gate: it opens existing files with
...
- `write_splashdown_env` (`provisioning.py:196`): builds `K=_env_quote(V)` lines and replaces the whole file. Empty `items` → empty file.
...
- `provision()` — resolve loop / per-type dispatch: `provisioning.py:26`
- Missing-recipe `FileNotFoundError`: `provisioning.py:33-34`
- Topo-sorted iteration: `provisioning.py:40`
- `set`-type missing-value error: `provisioning.py:74`
- `write_outputs()` — writer grouping + dispatch: `provisioning.py:87`
- `splashdown-env` truncate guard: `provisioning.py:99`
