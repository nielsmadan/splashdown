...
The instance name is the isolation key. `_default_sim_name` (`devices.py:36`) builds
...
own several configs (default, lowest-supported, …). `_resolve_device_name` (`devices.py:53`)
...
`_sanitize_avd_name` (`devices.py:46`) because `avdmanager` only accepts `[A-Za-z0-9._-]`; the
`/` separators become `_`. A leading `-` is rejected outright (`devices.py:67`) so the name can
...
- `_ios_find_device_by_name` (`devices.py:88`) scans every runtime bucket of
  ...
  `(udid, state)`.
- Version selection: `_ios_latest_runtime` (`devices.py:98`) and
  `_ios_latest_runtime_version` (`devices.py:108`) sort available runtimes by
  `_version_tuple` (`devices.py:118`) — which splits `"18.5"` into `(18, 5)` so `19.0 > 9.0`
  ...
  identifier. `_ios_runtime_identifier` (`devices.py:139`) maps a pinned version back to the
  CoreSimulator identifier (`18.5` → `…SimRuntime.iOS-18-5`).
- `_ios_device_type_identifier` (`devices.py:144`) resolves the `model` field, or defaults to
  the lexically-last `iPhone … Pro` device type when none is given.
- `ios_ensure` (`devices.py:162`) is find-or-create: returns the existing sim's `(udid, state)`
  if present, otherwise `simctl create` and returns `(udid, "Shutdown")` — creation never boots.
- `ios_boot` / `ios_shutdown` / `ios_destroy` (`devices.py:184`, `:199`, `:207`) wrap
  `boot`/`shutdown`/`delete`. Boot tolerates the benign "current state: Booted" race from a
  concurrent boot; shutdown first checks `_ios_current_state` (`devices.py:644`) to skip the
...
- `android_ensure` (`devices.py:286`) is find-or-create via `avdmanager create avd … --force`,
  feeding `\n` on stdin to decline the custom-hardware-profile prompt; defaults `device` to
  `pixel_9` and `image` to the latest.
- Boot is async: `android_boot` (`devices.py:341`) spawns `emulator -avd <name>` detached
  (`start_new_session=True`, output to a per-AVD log under `REGISTRY_DIR`), then polls
  `_android_running_serial` (`devices.py:309`) for up to 60s. That matcher is the load-bearing
...
`_devicectl_json` (`devices.py:386`) wraps `xcrun devicectl … --json-output -` (Xcode 15+) the
same way `_xcrun_json` wraps simctl. `_ios_physical_devices` (`devices.py:407`) gates on
...
`{id (udid), name, platform: "ios"}`. `_android_physical_devices` (`devices.py:435`) parses
...
(`devices.py:526`) maps the same match to `connected`/`absent`/`ambiguous` for `splash targets`.
...
row never drifts, so a pin is reconciled into existence once and then left alone forever; only
`latest` variants get destroyed+recreated when Xcode/SDK moves the floor.

The `if not stale:` block contains a `row is None` re-check that "never fires" — `stale` is True
whenever `row is None`, so the guard exists purely to narrow the Optional for mypy (`devices.py:570`).
...
(`devices.py:513`, `:575`, `:598`). Consumers must know which to read based on `kind`.
...
- `ensure_physical` — `devices.py:493` — physical-device resolution → `info`.
...
- **`_resolve_device_name` renders name templates with an EMPTY resources scope.** It calls
  `_make_scope(cwd, branch, {})` (`devices.py:63`), so a template like
...
- **`row.ios` is overloaded.** For emulator rows the `ios` column holds the *Android image*, and
  ...
  read those columns through that double meaning (`_device_status_for_row`, `devices.py:657`).
...
- **Orphan vs stale are different.** `_is_orphan_device` (`devices.py:254`) flags a registry row
