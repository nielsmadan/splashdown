...
def sync_agent_guidance(cwd: Path, recipe: Recipe) -> list[str]:
    """The guidance files this call rewrote, so init can report them as changed."""
    ...
    for name in _AGENT_FILES:
        ...
        if name == "CLAUDE.md" and agents_exists and _AGENTS_IMPORT_RE.search(text):
            desired = ""
        if _apply_managed_block(path, text, desired):
            written.append(name)
    return written
...
def _apply_managed_block(path: Path, text: str, block: str) -> bool:
    state = _managed_state(text)
    if state == _MALFORMED:
        marker = _generated_marker(text)
        if marker is None:
            _refuse(path, "its Splashdown guidance markers are malformed")
            return False
    ...
    if marker is not None:
        _report_generated_file(path, marker, block=block, state=state)
        return False
    if not _write_agent_file(path, updated):
        return False
...
def _replace_managed_block(text: str, block: str, state: str) -> str:
    ...
    if state == _ABSENT:
        if not block:
            return text
        separator = _paragraph_separator(text, newline)
...
def _paragraph_separator(text: str, newline: str) -> str:
    """A managed block appended straight after a paragraph reads as part of it in
    strict Markdown, so the block always opens after a blank line."""
    if not text:
        return ""
    if not text.endswith(newline):
        return newline * 2
    return "" if text.endswith(newline * 2) else newline


def _refuse(path: Path, reason: str) -> None:
    print(f"warning: left {path.name} alone: {reason}", file=sys.stderr)
...
def _read_agent_file(path: Path) -> str | None:
    try:
        ...
        if stat.S_ISLNK(current.st_mode):
            _refuse(path, "it is a symlink")
            return None
        if not stat.S_ISREG(current.st_mode):
            _refuse(path, "it is not a regular file")
            return None
        ...
        _refuse(path, f"it could not be read ({error})")
        return None
    try:
        return data.decode("utf-8")
    except UnicodeDecodeError:
        _refuse(path, "it is not UTF-8")
        return None
...
def _write_agent_file(path: Path, text: str) -> bool:
    try:
        ...
        _refuse(path, f"it could not be updated ({error})")
        return False
    return True
