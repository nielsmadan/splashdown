...
def _catalog(parsed_args):
    """Load the merged recipe+local device catalog for the completion cwd.
...
def variant_completer(prefix, parsed_args, **kwargs):
    """Slot 2: variant names for the typed/inferred device type."""
    try:
...
def device_arg_completer(prefix, parsed_args, **kwargs):
