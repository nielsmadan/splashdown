...
_LOADOUT_HEADER = (
    "<!-- Generated by loadout from skills/longshot/. Edits here are replaced on the next sync. -->"
)


def _vite_recipe(root: Path, project: dict[str, object] | None = None) -> sd.Recipe:
    recipe = _recipe(
        root,
        {"main": (".", "vite", ["WEB_DEV_PORT"])},
        {"WEB_DEV_PORT": {"type": "port", "range": [5174, 5200]}},
    ...
    return sd.Recipe(
        {
            "project": {"workspace": "single", "loader": "none", **project},
            "apps": {"main": {"path": ".", "profile": "vite", "resources": ["WEB_DEV_PORT"]}},
            "resources": {"WEB_DEV_PORT": {"type": "port", "range": [5174, 5200]}},
        },
        root / sd.RECIPE_NAME,
    )


def test_sync_leaves_an_externally_generated_file_byte_identical(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    original = f"{_LOADOUT_HEADER}\n\n# Rules\n".encode()
    path.write_bytes(original)
    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))
    assert path.read_bytes() == original
    err = capsys.readouterr().err
    assert "warning: left AGENTS.md alone: it is generated by another tool" in err
    assert "add the Splashdown guidance block to the source that generates AGENTS.md" in err


def test_sync_edits_a_file_that_only_mentions_generated_text(tmp_path):
    path = tmp_path / "AGENTS.md"
    path.write_text("# Rules\n\nThe client is generated by openapi-codegen; do not edit it.\n")
    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))
    text = path.read_text()
    assert "Framework: `vite`" in text
    assert "The client is generated by openapi-codegen; do not edit it." in text


def test_guidance_names_the_configured_env_destination(tmp_path):
    text = sd.render_agent_guidance(
...
def test_guidance_names_the_default_env_destination(tmp_path):
    text = sd.render_agent_guidance(tmp_path, _vite_recipe(tmp_path))
    assert "`splashdown.env`" in text


def test_env_file_text_cannot_inject_markers_or_markdown(tmp_path):
    marker = "<!-- <<< splashdown-managed agent-guidance <<< -->"
    recipe = _vite_recipe(tmp_path, {"env_file": f"config/`dev`.env\n{marker}"})
    text = sd.render_agent_guidance(tmp_path, recipe)
    assert text.count("<!-- >>> splashdown-managed agent-guidance >>> -->") == 1
    assert text.count(marker) == 1
    assert "config/`dev`.env\\n&lt;!-- &lt;&lt;&lt;" in text


def test_deinit_leaves_an_externally_generated_file_untouched(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    original = (
        f"{_LOADOUT_HEADER}\n\n# Rules\n"
        f"{sd.render_agent_guidance(tmp_path, _vite_recipe(tmp_path))}\n"
    ).encode()
    path.write_bytes(original)
    sd.remove_agent_guidance(tmp_path)
    assert path.read_bytes() == original
    err = capsys.readouterr().err
    assert "warning: left AGENTS.md alone: it is generated by another tool" in err
    assert "remove the Splashdown guidance block from the source that generates AGENTS.md" in err
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "<!-- auto-generated from templates/agents.md.j2 -->",
    ],
)
def test_sync_recognizes_common_generator_headers(tmp_path, capsys, header):
    path = tmp_path / "AGENTS.md"
    original = f"{header}\n\n# Rules\n".encode()
    path.write_bytes(original)
    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))
    assert path.read_bytes() == original
    assert (
        "warning: left AGENTS.md alone: it is generated by another tool" in capsys.readouterr().err
    )


def test_sync_edits_a_file_whose_generator_header_is_not_at_the_top(tmp_path):
    path = tmp_path / "AGENTS.md"
...
def test_sync_is_quiet_about_a_generated_file_whose_block_is_current(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    recipe = _vite_recipe(tmp_path)
    block = sd.render_agent_guidance(tmp_path, recipe)
    path.write_text(f"{_LOADOUT_HEADER}\n\n# Rules\n{block}\n")
    sd.sync_agent_guidance(tmp_path, recipe)
    assert capsys.readouterr().err == ""


def test_sync_reports_a_generated_claude_file_that_still_carries_a_block(tmp_path, capsys):
    (tmp_path / "AGENTS.md").write_text("# Shared\n")
    claude = tmp_path / "CLAUDE.md"
    recipe = _vite_recipe(tmp_path)
    ...
    err = capsys.readouterr().err
    assert "warning: left CLAUDE.md alone: it is generated by another tool" in err
    assert "remove the Splashdown guidance block from the source that generates CLAUDE.md" in err


def test_sync_prints_the_block_a_generated_file_needs(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    recipe = _vite_recipe(tmp_path)
    path.write_text(f"{_LOADOUT_HEADER}\n\n# Rules\n")
    sd.sync_agent_guidance(tmp_path, recipe)
    err = capsys.readouterr().err
    assert "  ----- begin Splashdown guidance block -----" in err
    assert f"\n{sd.render_agent_guidance(tmp_path, recipe)}\n" in err
    assert "  ----- end Splashdown guidance block -----" in err


def test_deinit_prints_no_block_for_a_generated_file(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    path.write_text(
        f"{_LOADOUT_HEADER}\n\n# Rules\n{sd.render_agent_guidance(tmp_path, _vite_recipe(tmp_path))}\n"
    )
    sd.remove_agent_guidance(tmp_path)
    err = capsys.readouterr().err
    assert "remove the Splashdown guidance block from the source that generates AGENTS.md" in err
    assert "----- begin Splashdown guidance block -----" not in err


def test_sync_reports_a_stale_block_in_a_generated_file_as_a_replacement(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    stale = sd.render_agent_guidance(tmp_path, _vite_recipe(tmp_path, {"env_file": "old/dev.env"}))
    original = f"{_LOADOUT_HEADER}\n\n# Rules\n{stale}\n"
    path.write_text(original)
    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))
    assert path.read_text() == original
    err = capsys.readouterr().err
    assert "replace the Splashdown guidance block in the source that generates AGENTS.md" in err
    assert "add the Splashdown guidance block" not in err


def test_sync_reports_a_generated_file_with_malformed_markers(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    original = (
        f"{_LOADOUT_HEADER}\n\n# Rules\n"
        "<!-- >>> splashdown-managed agent-guidance >>> -->\nbroken\n"
    ...
    assert path.read_text() == original
    err = capsys.readouterr().err
    assert "warning: left AGENTS.md alone: its Splashdown guidance markers are malformed" in err
    assert "is generated by another tool" in err
    assert "repair the Splashdown guidance markers in the source that generates AGENTS.md" in err


def test_deinit_asks_a_generated_file_with_malformed_markers_to_remove_them(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    original = (
        f"{_LOADOUT_HEADER}\n\n# Rules\n"
        "<!-- >>> splashdown-managed agent-guidance >>> -->\nbroken\n"
...
def test_sync_recognizes_a_generator_header_after_frontmatter(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    original = f"---\ntitle: Agent rules\nowner: platform\n---\n\n{_LOADOUT_HEADER}\n\n# Rules\n"
    path.write_text(original)
    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))
...
def test_sync_edits_a_file_whose_frontmatter_is_never_closed(tmp_path):
    path = tmp_path / "AGENTS.md"
    path.write_text(f"---\ntitle: Agent rules\n\n{_LOADOUT_HEADER}\n\n# Rules\n")
    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))
    assert "Framework: `vite`" in path.read_text()
...
def test_guidance_never_directs_agents_to_an_unimplemented_exec_command(tmp_path):
    assert "splash exec" not in sd.render_agent_guidance(tmp_path, _vite_recipe(tmp_path))


def test_render_is_empty_when_no_app_declares_a_port(tmp_path):
    recipe = _recipe(tmp_path, {"main": (".", "flutter", [])}, {})
    assert sd.render_agent_guidance(tmp_path, recipe) == ""


def test_sync_removes_the_block_when_no_app_declares_a_port(tmp_path, capsys):
    path = tmp_path / "AGENTS.md"
    path.write_text("# Rules\n")
    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))
    assert "Framework: `vite`" in path.read_text()
...
def test_sync_replaces_a_block_written_by_an_older_version(tmp_path):
    ...
    path.write_text(
        ...
        "## Splashdown\n"
        "\n"
        "Run `splash init --rescan`, then read the ports from `splashdown.env`.\n"
        "<!-- <<< splashdown-managed agent-guidance <<< -->\n"
        "# After\n"
    )
...
def test_sync_opens_the_block_after_a_blank_line(tmp_path):
    path = tmp_path / "AGENTS.md"
    path.write_text("# Rules\n\nA paragraph that ends the file.\n")

    sd.sync_agent_guidance(tmp_path, _vite_recipe(tmp_path))

    text = path.read_text()
    assert "A paragraph that ends the file.\n\n<!-- >>> splashdown-managed" in text


def test_sync_does_not_add_a_second_blank_line(tmp_path):
    path = tmp_path / "AGENTS.md"
    path.write_text("# Rules\n\n")
